...
public class Blowfish {
	...
	private BlowfishCBC mBfish;
	private byte[] iv;

	public Blowfish(byte[] passwd, byte[] iv) {
		mBfish = new BlowfishCBC(passwd, iv);
		this.iv = iv;
	}

	public Blowfish(byte[] passwd) {
		mBfish = new BlowfishCBC(passwd);
		this.iv="0".getBytes();
	...
	public byte[] decrypt(byte[] sourceBytes, byte[] iv) {
		mBfish.decrypt(sourceBytes);
		return sourceBytes;
	}

	public byte[] encrypt(byte[] sourceBytes, byte[] iv) {
		mBfish.encrypt(sourceBytes);
		return sourceBytes;
	}
	...
	private static class BlowfishECB {
		...
		public BlowfishECB(byte[] bfkey) {
			...
			mPbox = new int[PBOX_ENTRIES];

			for (nI = 0; nI < PBOX_ENTRIES; nI++) {
				mPbox[nI] = PBOX_INIT[nI];
			}

			mSbox1 = new int[SBOX_ENTRIES];
			mSbox2 = new int[SBOX_ENTRIES];
			mSbox3 = new int[SBOX_ENTRIES];
			mSbox4 = new int[SBOX_ENTRIES];

			for (nI = 0; nI < SBOX_ENTRIES; nI++) {
				mSbox1[nI] = SBOX_INIT_1[nI];
				mSbox2[nI] = SBOX_INIT_2[nI];
				mSbox3[nI] = SBOX_INIT_3[nI];
				mSbox4[nI] = SBOX_INIT_4[nI];
			}
			...
			for (nI = 0; nI < PBOX_ENTRIES; nI += 2) {
				...
				mPbox[nI] = (int) (lZero >>> 32);
				mPbox[nI + 1] = (int) (lZero & 0x0ffffffffL);
			}
			for (nI = 0; nI < SBOX_ENTRIES; nI += 2) {
				lZero = encryptBlock(lZero);
				mSbox1[nI] = (int) (lZero >>> 32);
				mSbox1[nI + 1] = (int) (lZero & 0x0ffffffffL);
			}
			for (nI = 0; nI < SBOX_ENTRIES; nI += 2) {
				lZero = encryptBlock(lZero);
				mSbox2[nI] = (int) (lZero >>> 32);
				mSbox2[nI + 1] = (int) (lZero & 0x0ffffffffL);
			}
			for (nI = 0; nI < SBOX_ENTRIES; nI += 2) {
				lZero = encryptBlock(lZero);
				mSbox3[nI] = (int) (lZero >>> 32);
				mSbox3[nI + 1] = (int) (lZero & 0x0ffffffffL);
			}
			for (nI = 0; nI < SBOX_ENTRIES; nI += 2) {
				lZero = encryptBlock(lZero);
				mSbox4[nI] = (int) (lZero >>> 32);
				mSbox4[nI + 1] = (int) (lZero & 0x0ffffffffL);
			}
		}
		...
		public void cleanUp() {
			...
			for (nI = 0; nI < SBOX_ENTRIES; nI++) {
				mSbox1[nI] = mSbox2[nI] = mSbox3[nI] = mSbox4[nI] = 0;
			}
		}
		...
		public static boolean selfTest() {
			...
			if ((tvT2[0] != tvP2[0]) || (tvT2[1] != tvP2[1])) {
				return false;
			}
		...
		protected long decryptBlock(long lCipherBlock) {
			...
			nHi ^= mPbox[17];
			nLo ^= (((mSbox1[nHi >>> 24] + mSbox2[(nHi >>> 16) & 0x0ff]) ^ mSbox3[(nHi >>> 8) & 0x0ff]) + mSbox4[nHi & 0x0ff])
					^ mPbox[16];
			nHi ^= (((mSbox1[nLo >>> 24] + mSbox2[(nLo >>> 16) & 0x0ff]) ^ mSbox3[(nLo >>> 8) & 0x0ff]) + mSbox4[nLo & 0x0ff])
					^ mPbox[15];
			nLo ^= (((mSbox1[nHi >>> 24] + mSbox2[(nHi >>> 16) & 0x0ff]) ^ mSbox3[(nHi >>> 8) & 0x0ff]) + mSbox4[nHi & 0x0ff])
					^ mPbox[14];
			nHi ^= (((mSbox1[nLo >>> 24] + mSbox2[(nLo >>> 16) & 0x0ff]) ^ mSbox3[(nLo >>> 8) & 0x0ff]) + mSbox4[nLo & 0x0ff])
			...
			return makeLong(nHi, nLo ^ mPbox[0]);
		}
	...
	public static class BlowfishCBC extends BlowfishECB {
		...
		public void getCBCIV(byte[] dest) {
			longToByteArray(mLCBCIV, dest, 0);
		}
		...
		public void setCBCIV(byte[] newCBCIV) {
			mLCBCIV = byteArrayToLong(newCBCIV, 0);
		}
